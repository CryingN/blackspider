    ...
    if args.file == None:
        if args.url == None:
            ...
            try:
                ...
                bk.dictionary.set("group",args.file,args.url.replace('=','[equal]'))
                bk.dictionary.set("test",args.file,args.url.replace('=','[equal]'))
        ...
        try:
            url_test = bk.dictionary.get("group",args.file).replace("[equal]","=")
            choose = input(bk.choose_+"从文件名搜索到url:{},是否使用(Y/n):".format(url_test))
            if choose in bk.choose_true:
                ...
                try:
                    ...
                    print(bk.warn+"字典部分设置失败,请检查url格式")
        except:
            if args.url == None:
...
if __name__ == "__main__":
    ...
    parser.add_argument("-q","--query", dest="query",default=False, help="获取当前保存文件名:-q name ,获取保存文件信息:-q all, 重置test部分:-q sync")
